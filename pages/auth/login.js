...
const Login = () => {
    ...
    const closeErrPemberitahuanLogin = (e) => {
        ...
        if(e.target.classList.contains('layar-aksi-login')){
            if(iconPemberitahuan.classList.contains('fa-circle-xmark') || iconPemberitahuan.classList.contains('fa-envelope-circle-check')){
    ...
    <AlertAuth />
        <section className="menu-login p-5 absolute top-0 bottom-0 right-0 left-0 z-10 flex items-center justify-center ">
            ...
            <form className="login opacity-0 animate-authMuncul py-6 px-6 sm:px-10 relative bg-white rounded-md text-xl dark:text-white dark:bg-biru-dark dark:shadow-popup" onSubmit={login}>
                <FontAwesome className='fa-solid fa-circle-user block text-center text-[5.5rem] mb-3 absolute -top-12 left-[50%] -ml-9' name='' />
                <h1 className="sm:text-2xl mb-6 text-center mt-8">Silahkan Login</h1>
                <section className='h-24 flex flex-col justify-between'>
                    <section className="flex justify-between items-center p-1 w-full">
                        <label htmlFor="email" className='text-base sm:text-xl'>Email <p className="ml-10 sm:mr-2 inline-block">:</p></label>
                        <input type="email" placeholder="email" className="border dark:border-abu-trans w-2/3 block pl-2 outline-none rounded-sm text-sm sm:text-lg py-1 sm:py-0 dark:bg-transparent" id="email" required defaultValue={email} onChange={e => setEmail(e.target.value)} />
                    </section>
                    <section className="flex justify-between items-center p-1 w-full">
                        ...
                        <section className='w-full flex justify-between items-center border dark:border-abu-trans'>
                            <input type="password" placeholder="password" className="w-[85%] block pl-2 outline-none rounded-sm text-sm py-1 sm:py-0 sm:text-lg dark:bg-transparent" id="password" required defaultValue={password} onChange={e => setPassword(e.target.value)}/>
                ...
                <button className="py-1.5 sm:py-2 w-full bg-biru rounded hover:bg-biru-hover dark:bg-biru-tua dark:hover:bg-biru-hover text-white mt-6 mb-2 text-lg" type="submit">Login</button>
                <section className='flex justify-between mt-1.5'>
                    <Link href={'/auth/register'}><a className='text-[0.785rem] sm:text-sm'>belum punya akun?</a></Link>
                    <p className='text-[0.785rem] sm:text-sm cursor-pointer' onClick={lupaPassword}>lupa password?</p>
                </section>
            </form>
        </section>
    </> 
